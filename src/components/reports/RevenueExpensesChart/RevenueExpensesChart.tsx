...
import type { RevenueExpensesChartData } from "@/types/dashboard";

export interface RevenueExpensesChartProps {
  chartData?: RevenueExpensesChartData;
  className?: string;
}

export const RevenueExpensesChart: React.FC<RevenueExpensesChartProps> = ({
  chartData = REVENUE_EXPENSES_CHART_DATA,
  className,
}) => {
  ...
  const data = chartData.dataPoints;
  ...
  const cardClassName = className ? `${styles.chartCard} ${className}` : styles.chartCard;

  return (
    <section className={cardClassName} aria-label={APP_STRINGS.headers.revenueVsExpenses}>
      <div className={styles.header}>
        <h2 className={styles.title}>{chartData.title || APP_STRINGS.headers.revenueVsExpenses}</h2>

        <div className={styles.legend}>
          <div className={styles.legendItem}>
            <span className={styles.legendDotRevenue} />
            <span>{chartData.revenueLabel || APP_STRINGS.chartLegends.revenue}</span>
          </div>
          <div className={styles.legendItem}>
            <span className={styles.legendLineExpenses} />
            <span>{chartData.expensesLabel || APP_STRINGS.chartLegends.expenses}</span>
          </div>
        </div>
        ...
        {activeTooltip && (
          <div className={styles.tooltipBox}>
            ...
            <span className={styles.tooltipMetric}>Rev: ৳{activeTooltip.revenue}k</span>
            <span className={styles.tooltipMetric}>Exp: ৳{activeTooltip.expenses}k</span>
          </div>
        )}
      </div>
          ...
          {points.map((p) => (
            <g key={p.month}>
              ...
              <circle
                ...
                onMouseLeave={() => setActiveTooltip(null)}
              />
              <text x={p.x} y={height - 4} className={styles.monthLabel}>
                {p.month}
              </text>
            </g>
          ))}
