...
export const TotalBalanceCard: React.FC<TotalBalanceCardProps> = ({
  ...
  return (
    <section className={cardClassName} aria-label={APP_STRINGS.headers.totalBalance}>
      <div className={styles.header}>
        <span className={styles.label}>{data.title}</span>
        <div className={styles.balanceRow}>
          <h2 className={styles.amount}>{formatCurrency(data.amount, data.currencySymbol)}</h2>
