...
export const EngagementRateCard: React.FC<EngagementRateCardProps> = ({
  ...
  const [activePeriod, setActivePeriod] = useState<"monthly" | "annually">(data.activePeriod);

  const currentDataset = data.periods[activePeriod] ?? [];
  const cardClassName = className ? `${styles.card} ${className}` : styles.card;

  return (
    <section className={cardClassName} aria-label={APP_STRINGS.headers.engagementRate}>
          ...
          return (
            <div key={`${activePeriod}-${item.month}`} className={styles.column}>
              <div className={styles.barTrack}>
                {item.highlightBadge && <span className={styles.badge}>{item.highlightBadge}</span>}
                <div
